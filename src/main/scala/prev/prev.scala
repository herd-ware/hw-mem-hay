 ...
 * -----                                                                       *
 * Last Modified: 2023-02-25 09:41:03 pm                                       *
...
class Prev (p: PrevParams) extends Module {
  val io = IO(new Bundle {
    ...
    val b_cache_rep   = Vec(p.nDomeSlct, new CacheRepIO(p))
  ...
  for (pp <- 0 until p.nPrevPort) {
    if (p.useDome) m_unit(pp).io.b_dome.get <> io.b_dome.get
    m_unit(pp).io.b_cbo <> io.b_cbo

    // ------------------------------
    //             PORT
    // ------------------------------
    if (p.pPrev(pp).useDomeSlct) m_unit(pp).io.i_slct.get := io.i_slct.get  
    ...
    if (!p.readOnly) {
      if (!p.pPrev(pp).readOnly) {
        ...
        for (d <- 0 until p.nDomeTag) {
          m_zero.get.io.b_port(pp)(d).ready := false.B
        }
      }
    }
      ...
      if (!p.pPrevBus(pp).readOnly) {
        if (p.useDomeSlct == p.pPrevBus(pp).useDomeSlct) {
          ...
          for (ds <- 0 until p.nDomeSlct) {
            io.b_next_data.get(pp).valid(ds) := m_unit(pp).io.b_next_data.get.valid(0) & (ds.U === m_unit(pp).io.b_next_data.get.dome.get)
            io.b_next_data.get(pp).data.get(ds) := m_unit(pp).io.b_next_data.get.data.get(0)
            when (ds.U === m_unit(pp).io.b_next_data.get.dome.get) {
              m_unit(pp).io.b_next_data.get.ready(0) := io.b_next_data.get(pp).ready(ds)
            }
          }
        }        
      } else {
        ...
        for (ds <- 0 until p.nDomeSlct) {
          io.b_next_data.get(pp).valid(ds) := false.B
        }        
      }      
    }
  ...
  //          CACHE REPLACE
  // ******************************
  val w_cache_rep_valid = Wire(Vec(p.nDomeSlct, Vec(p.nPrevPort, Bool())))
  val w_cache_rep_av = Wire(Vec(p.nDomeSlct, Vec(p.nPrevPort, Bool())))

  // ------------------------------
  //            DEFAULT
  // ------------------------------
  for (ds <- 0 until p.nDomeSlct) {
    io.b_cache_rep(ds) := DontCare
    io.b_cache_rep(ds).valid := false.B
  }
  ...
  for (pp <- 0 until p.nPrevPort) {
    ...
    if (p.useDomeSlct) {
      for (ds <- 0 until p.nDomeSlct) {
        w_cache_rep_valid(ds)(pp) := m_unit(pp).io.b_cache_rep.valid & (m_unit(0).io.b_cache_rep.dome.get === ds.U)
      }
    } else {
  ...
  // ------------------------------
  for (ds <- 0 until p.nDomeSlct) {
    w_cache_rep_av(ds)(0) := ~w_cache_rep_valid(ds)(0)
    when (w_cache_rep_valid(ds)(0)) {
      m_unit(0).io.b_cache_rep <> io.b_cache_rep(ds)
    }

    for (pp <- 1 until p.nPrevPort) {
      w_cache_rep_av(ds)(pp) := w_cache_rep_av(ds)(pp - 1) & ~w_cache_rep_valid(ds)(pp)
      when (w_cache_rep_av(ds)(pp - 1) & w_cache_rep_valid(ds)(pp)) {
        m_unit(pp).io.b_cache_rep <> io.b_cache_rep(ds)
      }
    }
  }
  ...
  var ndep: Int = 0
  for (pp <- 0 until p.nPrevPort) {
    for (ds <- 0 until p.pPrevBus(pp).nDomeSlct) {
      // ------------------------------
      //            REQ STAGE
      // ------------------------------
      m_dep_req.io.b_back(ndep + ds) <> m_unit(pp).io.b_dep_back(ds)
      m_dep_req.io.b_bus(ndep + ds) <> m_unit(pp).io.b_dep_bus(ds)
      if (p.useReqReg) m_dep_req.io.b_reg.get(ndep + ds) <> m_unit(pp).io.b_dep_req.get(ds)
      if (p.useAccReg) {
        m_dep_req.io.i_acc(ndep + ds) := m_unit(pp).io.b_dep_acc.get(ds).reg
      } else {
        m_dep_req.io.i_acc(ndep + ds) := m_unit(pp).io.b_dep_read(ds).reg
      }
      ...
      // ------------------------------
      if (p.useAccReg) {
        m_dep_acc.get.io.b_acc(ndep + ds) <> m_unit(pp).io.b_dep_acc.get(ds)
        m_dep_acc.get.io.i_read(ndep + ds) := m_unit(pp).io.b_dep_read(ds).reg
      }
      ...
      // ------------------------------
      if (p.useAccReg) {
        m_dep_read.io.i_acc(ndep + ds) := m_unit(pp).io.b_dep_acc.get(ds).reg
      } else if (p.useReqReg) {
        m_dep_read.io.i_acc(ndep + ds) := m_unit(pp).io.b_dep_req.get(ds).reg
      } else {
        m_dep_read.io.i_acc(ndep + ds) := m_unit(pp).io.b_dep_bus(ds).reg
      }
      m_unit(pp).io.i_dep_rw(ds) := m_dep_read.io.o_dep_rw(ndep + ds)
      m_dep_read.io.b_read(ndep + ds) <> m_unit(pp).io.b_dep_read(ds)
      if (p.useAckReg && !p.readOnly) m_dep_read.io.i_ack.get(ndep + ds) := m_unit(pp).io.o_dep_ack.get(ds)
    }

    ndep = ndep + p.pPrevBus(pp).nDomeSlct
  }  
  ...
  if (p.useDome) {
    for (d <- 0 until p.nDome) {
      ...
      for (pp <- 0 until p.nPrevPort) {
        w_free(pp) := m_unit(pp).io.b_dome.get(d).free
      }

      io.b_dome.get(d).free := w_free.asUInt.andR
    }
  }
}
